<h4>Tambah Data</h4>
<br>
<form action="index.php?mod=siswa&page=save" method="POST">
    <div class="col-lg-6">
        <div class="form-group">
            <label for="">Nama Mahasisswa</label>
            <input type="text" name="nama" required value="<?=(isset($_POST['nama']))?$_POST['nama']:'';?>" class="form-control">
            <span class="text-danger"><?=(isset($err['nama']))?$err['nama']:'';?></span>
        </div>
        <div class="form-group">
            <label for="">Nim</label>
            <input type="text" name="nim" required value="<?=(isset($_POST['nim']))?$_POST['nim']:'';?>" class="form-control">
            
        </div>
        <div class="form-group">
            <label for="">Prodi</label>
            <input type="text" name="prodi" required value="<?=(isset($_POST['prodi']))?$_POST['prodi']:'';?>" class="form-control">
            
        </div>
        <div class="form-group">
            <label for="">No Handphone</label>
            <input type="number" name="no_hp" required value="<?=(isset($_POST['no_hp']))?$_POST['no_hp']:'';?>" class="form-control">
            
        </div>
        <div class="form-group">
            <button type="submit" class="btn btn-primary">Save</button>
        </div>
    </div>
</form>
